...
- **SavedVariables are written and never loaded, for every addon** (verified
  in game 2026-09-21, character "Coffee Issues" on realm "Classic Beta PvE
  2"). Per character first: after a `/reload`, `GoblinPSCharDB` was `nil`
  while its file on disk, rewritten by that same reload, held the two learned
  flight paths. GoblinPS then moved everything to the account-wide save, and
  that was wrong too. The first check read `/run print(GoblinPSDB)` after a
  reload and saw a table -- but GoblinPS creates that table itself at
  `PLAYER_LOGIN`, so the check could not tell a loaded save from a fresh one.
  The decisive test: `/run GoblinPSDB.test = 42`, `/reload`, and the file on
  disk held `["test"] = 42` while `GoblinPSDB.test` printed `nil`. A control
  with no GoblinPS code in it failed the same way: Wowhead Looter (a normal
  folder, not a junction) forgets its minimap button setting on every
  reload, and its own save logs exactly one login event, never accumulating.
  So the cause is the client, not the junction, the character's two-word
  name or anything GoblinPS does. The account folder's odd layout (realm
  folders `70/` and `Classic Beta PvE 2/` side by side, character folders cut
  off at the space) and a stray `WTF/Account/SavedVariables/` folder holding
  only Blizzard files are noted, not explained. GoblinPS keeps the
  account-wide save keyed by `"Name-Realm"`, which is the right shape once
  loading works; until then learned flight paths and a trip in progress last
  one session. That makes three things on this client that are present and
  do not work.
