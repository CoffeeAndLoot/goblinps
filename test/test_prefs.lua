return function(h, loaded)
    ...
    h.describe("the scrolling text", function()
        h.it("lists four speeds, with a step for every one but off", function()
            h.eq(table.concat(Prefs.SCROLL, ","), "off,slow,normal,fast")
            h.eq(Prefs.SCROLL_STEP.off, nil, "off never moves")
            h.eq(Prefs.SCROLL_STEP.slow, 0.3)
            h.eq(Prefs.SCROLL_STEP.normal, 0.2)
            h.eq(Prefs.SCROLL_STEP.fast, 0.12)
        end)
        h.it("defaults to normal and keeps a choice from the list", function()
            h.eq(Prefs.Init(nil).scroll, "normal")
            for _, name in ipairs(Prefs.SCROLL) do
                h.eq(Prefs.Init({ scroll = name }).scroll, name)
            end
        end)
        h.it("repairs anything not in the list to normal", function()
            h.eq(Prefs.Init({ scroll = "warp" }).scroll, "normal")
            h.eq(Prefs.Init({ scroll = 2 }).scroll, "normal")
            h.eq(Prefs.Init({ scroll = {} }).scroll, "normal")
        end)
        h.it("Reset puts it back to normal", function()
            local db = Prefs.Init({ scroll = "off" })
            Prefs.Reset(db)
            h.eq(db.scroll, "normal")
        end)
    end)
